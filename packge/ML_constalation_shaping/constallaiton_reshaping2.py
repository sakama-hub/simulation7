...
class constallation_reshaping:
    """
    Class for reshaping the points of symbol
    """

    def __init__(self, shaping_points, M, ratio=1.0, step_size=0.01):
        """
        PARAMERTERS
        ----------
        ...
        ratio : float (default 1.0)
                parametor to define the initial ratio between attractive force and repulsive force(repuls / attract)
        -------------------------------------------
        RETURN
        ----------
        nothing
        ...
        NOTE
        ----------
        this class will be modified from constaltion rashaping class in terms of the size of attractive force.
        the size of attractive force is higher.
        -------------------------------------------
        Changing points from ordinal module
        ----------
        * due to adding a argument of ratio between attractive force and repulsive force
        """
        ...
        for key in self.shaping_points.keys():
            for key_ in neighbor_symbols[key]:
                ...
                # arise attractive force
                    ...
                    new_shaping_points[key] = new_shaping_points[key] + self.ratio * self.step_size * self.unit_vector(self.shaping_points[key], self.shaping_points[key_])
    ...
    def updata_shaping_points(self, shaping_points):
        """Updata the shaping points."""
        self.shaping_points = shaping_points

    def updata_ratio(self, ratio):
        """Updata ratio between attractibe force and repulsive force."""
        self.ratio = ratio

    def updata_stepsize(self, stepsize):
        """Updata the step size."""
...
if __name__ == "__main__":
    ...
    shape = constallation_reshaping(step_size, shaping_points, 4)
